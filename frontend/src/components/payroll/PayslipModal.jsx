...
SALARY
Gross: ${moneyPrefix} ${fmt(b.grossSalary)}
Deductions:
${detailedDeductions.map(([label, amount]) => `- ${label}: ${moneyPrefix} ${fmt(amount)}`).join('\n')}
Total Deductions: ${moneyPrefix} ${fmt((b.totalDeductions || 0) + (b.salaryAdvance || 0))}
Net Salary: ${moneyPrefix} ${fmt(b.netSalary)}
---------------------
...
export default function PayslipModal({
  ...
  }, [company, payrollRow, periodLabel, breakdown, effectiveLateDetails, currency, showIndiaStatutory]);
  ...
  const absentDeductFormula =
    absentDaysNum > 0 && absentDeductNum > 0
      ? `${formatHours(absentDaysNum)} days x ${fmtSym(absentPerDayRate)}/day = ${fmtSym(absentDeductNum)}`
      : null;
          ...
          <section className="grid gap-4 rounded-xl border border-slate-100 bg-slate-50/80 px-4 py-3 text-[11px] text-slate-700 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <div className="flex justify-between">
                ...
                <span>{fmtSym(attendanceDetails?.basic_salary)}</span>
              </div>
            </div>
            <div className="space-y-1.5">
              <div className="flex justify-between">
              ...
              <div className="space-y-1.5">
                {isCompletePeriod && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{fmtSym(breakdown.employee?.basic_salary)}</span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span>Travel Allow.</span>
                  <span className="font-medium">
                    {fmtSym(breakdown.breakdown?.travelAllowance)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Other Allow.</span>
                  <span className="font-medium">
                    {fmtSym(breakdown.breakdown?.otherAllowance)}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-emerald-700">
                    {fmtSym(breakdown.breakdown?.paidLeaveEncashmentAmount)}
                  </span>
                </div>
                <div className="flex justify-between">
            ...
            <div>
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-emerald-700">
                    −{fmtSym(breakdown.breakdown?.permissionOffsetAmount)}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-amber-700">
                    {fmtSym(breakdown.breakdown?.salaryAdvance)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Pending Advance Balance</span>
                  <span className="font-medium text-amber-700">
                    {fmtSym(breakdown.breakdown?.pendingAdvanceBalance)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Absent Deduct.</span>
                  <span className="font-medium text-amber-700">
                    {fmtSym(breakdown.breakdown?.absenceDeduction)}
                  </span>
                </div>
                {absentDeductFormula && (
                  <p className="payslip-print-hidden -mt-0.5 text-[10px] text-slate-500">
                ...
                {showIndiaStatutory && (
                  <>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-amber-700">
                        {fmtSym(breakdown.breakdown?.pfDeduction)}
                      </span>
                    </div>
                  </>
                )}
          ...
          {!!(breakdown.advance_repayments || []).length && (
            <section className="payslip-print-hidden rounded-xl border border-slate-100 bg-white px-4 py-3 text-[11px] text-slate-700">
                  ...
                  <div key={`${repayment.loan_id}-${index}`} className="flex items-start justify-between gap-3 border-b border-slate-100 pb-1.5">
                    <div>
                      <p className="font-medium text-slate-800">
                        Loan #{index + 1} ({fmtSym(repayment.original_loan_amount)} on {repayment.loan_date || '—'})
                      </p>
                      <p className="text-[10px] text-slate-500">
                        Outstanding after: {fmtSym(repayment.outstanding_balance_after)}
                      </p>
                    </div>
                    <p className="font-semibold text-amber-700">-{fmtSym(repayment.this_month_deduction)}</p>
                  </div>
                ))}
              </div>
              <div className="mt-2 flex justify-between border-t border-slate-200 pt-2 font-semibold text-slate-900">
                ...
                <span>-{fmtSym(breakdown.breakdown?.salaryAdvance)}</span>
              </div>
            </section>
          )}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="text-base font-semibold text-slate-900">
                  {fmtSym(breakdown.breakdown?.grossSalary)}
                </p>
              </div>
              <div>
                <p className="text-xs font-semibold text-slate-700">Total Deductions</p>
                <p className="text-sm font-semibold text-amber-700">
                  {fmtSym(breakdown.breakdown?.totalDeductions + breakdown.breakdown?.salaryAdvance)}
                </p>
              </div>
              <div>
                <p className="text-xs font-semibold text-slate-700">NET SALARY</p>
                <p className="text-lg font-bold text-emerald-700">
                  {fmtSym(breakdown.breakdown?.netSalary)}
                </p>
              </div>
              {showIndiaStatutory && (
                <div>
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-600">
                  Paid: {fmtSym(paymentSummary?.total_paid ?? payrollRow?.total_paid ?? 0)}
                  {' · '}
                  Balance: <span className="font-semibold text-amber-700">{fmtSym(balanceDue)}</span>
                </p>
                  ...
                  <tbody>
                    {(paymentSummary.payments || []).map((p) => (
                      <tr key={p.id} className="border-b border-emerald-50">
                        ...
                        <td className="py-1 text-right font-medium">{fmtSym(p.amount)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
