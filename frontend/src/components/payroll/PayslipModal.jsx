...
Total Deductions: ₹${formatMoney((b.totalDeductions || 0) + (b.salaryAdvance || 0))}
...
export default function PayslipModal({
  ...
  const handleSendPayslipWhatsApp = () => {
    if (!employeeWhatsAppNumber) return;
    openWhatsAppChat(employeePhone, payslipWhatsAppText);
  };

  const handleShareWhatsappSummary = () => {
    const waUrl = `https://wa.me/?text=${encodeURIComponent(payslipWhatsAppText)}`;
